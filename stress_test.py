...
import google.gflags as flags
import stress_test_common
import stress_test_pb2

try:
  if "pexpect" not in globals():
    import pexpect  # pylint:disable=g-import-not-at-top
except ImportError:
  print "pexpect not supported, falling back to subprocess for tailing logs"
  pexpect = None


FLAGS = flags.FLAGS
flags.DEFINE_string("notification_address", "mokani@google.com",
                    "Email address where to send notification events. Will "
                    "default to $USER@google.com if not provided. No emails "
                    "will be sent if supress_notification_emails is True.")
flags.DEFINE_bool("supress_notification_emails", False,
...
flags.DEFINE_string("email_sender_address", "speechmicrostresstest@gmail.com",
                    "Account to use for sending notification emails.")
flags.DEFINE_string("email_sender_password", "okgoogle",
...
class ProcessLogger(threading.Thread):
  ...
  def run(self):
    ...
    while self.restart_process:
      self.lock.release()
      if not first_run:
        logging.info("Restarting process " + " ".join(self.command))
      ...
      try:
        ...
        for line in output_source:
          # If the process we're logging likes to repeat it's output, we need to
          ...
          if self.output_fp:
            self.output_fp.write(line.rstrip())
  ...
  def StopLogging(self):
    if self.process:
      ...
      self.lock.release()
      if pexpect:
        self.process.kill(pexpect.signal.SIGHUP)
...
class Device(object):
  ...
  def __init__(self, serial_number, output_root, test_events):
  ...
  def __init_async__(self):
    ...
    for file_to_watch in config.file_to_watch:
      ...
      if file_to_watch.source == "LOGCAT":
        command = ["adb", "-s", self.serial_number, "logcat",
                   "-v", "usec", ""]
        command.extend(["%s:S" % tag for tag in config.tag_to_supress])
        name = "logcat_" + self.serial_number
      else:
        command = ["adb", "-s", self.serial_number, "shell",
                   "while : ; do cat %s ; done" % file_to_watch.source]
        name = "%s_%s" % (os.path.basename(file_to_watch.source),
                          self.serial_number)
  ...
  def __ValuesInEval(self):
    values_in_eval = {key: value for key, value
                      in self.event_counter.iteritems()}
    for key, value in self.iterations_since_event.iteritems():
    ...
    if command[0] in {"bugreport", "root", "wait-for-device", "shell",
      ...
      for source_file in files:
        ...
        if FLAGS.delete_data_dir:
          subprocess.check_output(["adb", "-s", self.serial_number, "shell", "rm",
                                   "-rf", source_file])
        return ""
    else:
  ...
  def GetSummaryLines(self):
    lines = [
        "Device %s" % self.name,
        "------------------------------------------------------------------",
        "|          Event Type          |  Total  | Iterations Since Last |",
        "------------------------------------------------------------------",
    ]
    for event, count in sorted(self.event_counter.items()):
      lines.append("|%-30.30s|% 9d|% 23d|" % (
...
class StressTest(object):
  ...
  def __init__(self, output_root, test_name):
    ...
    for serial_number in self.GetActiveSerialNumbers():
      self.devices.append(Device(serial_number, output_root, self.events))
    if not self.devices:
  ...
  def Start(self):
    ...
    while True:
      ...
      RunAsyncCommand(devices, ["shell", "log", "-t", "STRESS_TEST",
                                ...
                                "/proc/timer_list | tail -n 1)" % iteration])
        ...
        if step.audio_file:
          ...
          if FLAGS.use_sox:
            subprocess.check_call(["sox", "-q",
                                   "-c", str(step.audio_file_num_channels),
                                   "-t", step.audio_file_format,
                                   "-r", str(step.audio_file_sample_rate),
      ...
      # TODO(thorntonc): Sometimes the logcat seems to get stuck and buffers for
      ...
      if iteration % FLAGS.print_summary_every_n == 0:
      ...
      if FLAGS.num_iterations:
        if iteration >= FLAGS.num_iterations:
          break
    SendNotificationEmail(
...
def main(unused_argv):
  ...
  try:
    if "adb" in subprocess.check_output(["ps", "-ale"]):
    ...
    # Don't care about this on windows, they're on their own.
    pass
